...
class MoesE2EPage:
    ...
    def order_placement(self):
        try:
            ...
            self.page.wait_for_selector(MoesE2ELocators.START_ORDER, timeout=10000)
            self.page.click(MoesE2ELocators.START_ORDER)

            logger.info("Waiting for store search input and filling address: 'Guam'")
            self.page.wait_for_selector(MoesE2ELocators.SEARCH_TEXT, timeout=10000)
            self.page.fill(MoesE2ELocators.SEARCH_TEXT, "Guam")

            logger.info("Waiting for and clicking address from suggestions")
            self.page.wait_for_selector(MoesE2ELocators.SELECT_ADDRESS_BTN, timeout=10000)
            self.page.click(MoesE2ELocators.SELECT_ADDRESS_BTN)

            logger.info("Waiting for and selecting location")
            self.page.wait_for_selector(MoesE2ELocators.SELECT_LOCATION, timeout=10000)
            self.page.click(MoesE2ELocators.SELECT_LOCATION)
            ...
            time.sleep(2)  # Let UI settle after cart click
            checkout_button.scroll_into_view_if_needed()
            checkout_button.click()
            time.sleep(10)  # Let checkout page load
